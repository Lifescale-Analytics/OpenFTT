...
import Print from "https://js.arcgis.com/4.33/@arcgis/core/widgets/Print.js";
import CoordinateConversion from "https://js.arcgis.com/4.33/@arcgis/core/widgets/CoordinateConversion.js";
import MapView from "https://js.arcgis.com/4.33/@arcgis/core/views/MapView.js";
import Sublayer from "https://js.arcgis.com/4.33/@arcgis/core/layers/support/Sublayer.js";
...
import * as locator from "https://js.arcgis.com/4.33/@arcgis/core/rest/locator.js";
import * as gsvc from "https://js.arcgis.com/4.33/@arcgis/core/rest/geometryService.js";
import * as geometryEngine from "https://js.arcgis.com/4.33/@arcgis/core/geometry/geometryEngine.js";
import BufferParameters from "https://js.arcgis.com/4.33/@arcgis/core/rest/support/BufferParameters.js";
import Graphic from "https://js.arcgis.com/4.33/@arcgis/core/Graphic.js";
import SpatialReference from "https://js.arcgis.com/4.33/@arcgis/core/geometry/SpatialReference.js";
import BasemapToggle from "https://js.arcgis.com/4.33/@arcgis/core/widgets/BasemapToggle.js";
import ScaleBar from "https://js.arcgis.com/4.33/@arcgis/core/widgets/ScaleBar.js";
import MapImageLayer from "https://js.arcgis.com/4.33/@arcgis/core/layers/MapImageLayer.js";
import * as urlUtils from "https://js.arcgis.com/4.33/@arcgis/core/core/urlUtils.js";
import * as webMercatorUtils from "https://js.arcgis.com/4.33/@arcgis/core/geometry/support/webMercatorUtils.js";
import Popup from "https://js.arcgis.com/4.33/@arcgis/core/widgets/Popup.js";
import TextSymbol from "https://js.arcgis.com/4.33/@arcgis/core/symbols/TextSymbol.js";
import * as reactiveUtils from "https://js.arcgis.com/4.33/@arcgis/core/core/reactiveUtils.js";
const response = await fetch('./config.json');
const data = await response.json();
import Portal from "https://js.arcgis.com/4.33/@arcgis/core/portal/Portal.js";
import OAuthInfo from "https://js.arcgis.com/4.33/@arcgis/core/identity/OAuthInfo.js";
import esriId from "https://js.arcgis.com/4.33/@arcgis/core/identity/IdentityManager.js";
import * as proximityOperator from "https://js.arcgis.com/4.33/@arcgis/core/geometry/operators/proximityOperator.js";
import * as intersectionOperator from "https://js.arcgis.com/4.33/@arcgis/core/geometry/operators/intersectionOperator.js";
  ...
  let row = resulttblhdr.insertRow(-1);
  var headerCell;
...
//   }
  ...
  function sldisttostation(poi) {
    ...
    for (var i = 0; i < stationList.length; i++) {
      ...
      var seg = new Polyline();
	    seg.spatialReference = new SpatialReference({wkid: mapSpatialReference });
  ...
  function showPOI() {
    ...
    var poi = gsvc.buffer(gsvcURL,params).then(function (poiPoints) {
      let poiBuffer = poiPoints[0];
      ltgLayer.add(
        new Graphic({
          geometry: poiBuffer,
  ...
  view.popup.actions = [copyLatLongAction];

  view.on("double-click", function (event) {
    event.stopPropagation();
    var maplong = event.mapPoint.longitude;
    var maplat = event.mapPoint.latitude;
    var lon = Math.round(maplong * 1000) / 1000;
    var lat = Math.round(maplat * 1000) / 1000;
  ...
  function copyLatLong() {
    //update coords for copy action
    const geom = view.popup.selectedFeature?.geometry?.latitude==null ? view.popup.location : view.popup.selectedFeature.geometry;
    document.getElementById("ltgLat").value = geom.latitude;
    document.getElementById("ltgLon").value = geom.longitude;
  ...
  function populateStationDropDown(lineGeometries) {
      ...
      gsvc.buffer(gsvcURL,params).then(function (lineBuffers) {
        let lineBuffer = lineBuffers[0];
  ...
  function setStructureDefinitionExpression(lineID) {
    //if the structure layer uses lineids for filtering, if not use proximity
      ...
      return gsvc.buffer(gsvcURL, params).then(function (lineBuffers){
        let lineBuffer = lineBuffers[0];
...
//   }
  ...
  function getSegmentEndPoints(lineGeometries) {
    ...
    var i = 0;
    //look for segment endpoints based on line geometry
    ...
    //   let lnn = parseInt(ln);
    //   if (lnn < lineGeometries.length - 1) {
    //     var segment1 = new Polyline({
    //       paths: lineGeometries[ln].paths,
    //       spatialReference: mapSpatialReference
    //     });
        
    //     for (let j = lnn+1; j < lineGeometries.length; j++) {
    //       var segment2 = new Polyline({
    //         paths: lineGeometries[lnn +1].paths,
    //         spatialReference: mapSpatialReference
    //       });
    
    //       var intersections = geometryEngine.intersectLinesToPoints(segment1, segment2);
    //       intersections.forEach(function(intersection) {
    //         var intPt = [intersection.x, intersection.y];
    //         var val = {
    //           id: i,
    //           line: ln,
    //           path: '0',
    //           startPt: segment1.paths[0][0],
    //           endPt: intPt,
    //           neighbors: null,
    //         };
    //         i++;
  ...
  $("#ltgAddress").autocomplete({
    source: function (request, response) {
      ...
      locator.suggestLocations(locatorURL, params).then((data) => {
    ...
    select: function (event, ui) {
      ...
      locator.addressToLocations(locatorURL, newaddress).then((results) => {
        if (results.length > 0) {
          var lat = document.getElementById("ltgLat");
          var lon = document.getElementById("ltgLon");
