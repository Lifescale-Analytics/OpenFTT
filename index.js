...
require([
  ...
  function setupEventListeners() {
    lineSelect.addEventListener("change", function () {
      ...
      if (lineID.length > 0) {
        //highlight line
        lineGeometries = setLineDefinitionExpression(lineID).then(
  ...
  function selectLine() {
    if (defaultLineId != "") {
      ...
      lineGeometries = setLineDefinitionExpression(defaultLineId).then(
        populateStationDropDown
      );
      //highlight structures
      structureGeometries = setStructureDefinitionExpression(defaultLineId);
      //highlights fault inidicators
  ...
  function processLightningResponse(rsp, starttimefield) {
    ...
    if (rspType == "success") {
      ...
      for (i = 0; i < r.length; i++) {
        ...
        if (maxDiff > 0) {
          var curdatetime =
            r[i].getElementsByTagName("date")[0].childNodes[0].nodeValue +
            " " +
            r[i].getElementsByTagName("time")[0].childNodes[0].nodeValue;
          var lTime = moment.tz(
            curdatetime,
            "MMM DD, YYYY HH:mm:ss:SSS",
            "GMT"
          );
          var curTime = lTime.clone().tz(timezone).format("HH:mm:ss.SSS");
          var curSecs = curTime.split(":")[2];
          if (Math.abs(curSecs - secs) < maxDiff) {
            var curlat = parseFloat(
              r[i].getElementsByTagName("lat")[0].childNodes[0].nodeValue
            );
            var curlon = parseFloat(
              r[i].getElementsByTagName("lon")[0].childNodes[0].nodeValue
            );
            var curSig = parseFloat(
              r[i].getElementsByTagName("signal")[0].childNodes[0].nodeValue
            ).toFixed(2);
            var semimajor = parseFloat(
              r[i].getElementsByTagName("semi-major-axis")[0].childNodes[0]
                .nodeValue
            );
            var semiminor = parseFloat(
              r[i].getElementsByTagName("semi-minor-axis")[0].childNodes[0]
                .nodeValue
            );
            var angle_rad = parseFloat(
              r[i].getElementsByTagName("angle")[0].childNodes[0].nodeValue
            );
            var formattedDateTime = moment
              .tz(lTime, "MM/DD/YYYY HH:mm:ss:SSS", timezone)
              .format("MM/DD/YYYY HH:mm:ss.SSS");
            ltgPoints.push({
              id: j,
              lat: curlat,
          ...
          var curdatetime =
            moment(document.getElementById(starttimefield).value).format(
              "MM/DD/yyyy"
            ) +
            " " +
            curTime;
          ...
          var formattedDateTime = moment
            .tz(curdatetime, "MM/DD/YYYY HH:mm:ss:SSS", timezone)
            .format("MM/DD/YYYY HH:mm:ss");
